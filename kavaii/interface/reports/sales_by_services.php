...
<?php
  ...
  if ($_POST['form_csvexport']) {
    ...
    header("Content-Disposition: attachment; filename=sales_by_services.csv");
    header("Content-Description: File Transfer");
    // CSV headers:
 
    // CSV headers:
    echo '"' . xl('Service Group') . '",';
    echo '"' . xl('Category') . '",';
	echo '"' . xl('Patients') . '",';
    echo '"' . xl('Encounters') . '",';
    echo '"' . xl('Amount') . '"'."\n";
...
<title><?php xl('Sales by Service Groups','e') ?></title>
</head>

<body leftmargin='0' topmargin='0' marginwidth='0' marginheight='0' class="body_top">

<span class='title'><?php xl('Report','e'); ?> - <?php xl('Sales by Service Groups','e'); ?></span>

<form method='post' action='sales_by_services.php' id='theform'>
...
<?php
  ...
  if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
    ...
    if ($INTEGRATED_AR) {
      $query = "SELECT b.date,sum(b.fee) fee,  b.code_type, b.code,count(distinct b.pid) pid, count(distinct b.encounter) encounter, b.units, " .
        "b.code_text, fe.date, fe.pc_catid,fe.facility_id, fe.invoice_refno,op.pc_catname,LEFT(encounter_ipop, 2) Category " .
        "FROM billing AS b " .
      ...
      $query .= " GROUP BY b.code_type,LEFT(encounter_ipop, 2)";
      //
      $res = sqlStatement($query);
      while ($row = sqlFetchArray($res)) {
		  	if ($_POST['form_csvexport']) {
   
    echo '"' . qescape($row['code_type']) . '",';
    echo '"' . qescape($row['Category']) . '",';
    ...
    echo '"' . oeFormatMoney($row['fee']) . '"'."\n";
		   ...
		   <tr bgcolor="#ddddff">
  <td class="detail">
   <?php echo display_desc($row['code_type']) ?>
  </td>
  <td align="right">
   <?php echo display_desc($row['Category']); ?>
  </td>
   <td align="right">
