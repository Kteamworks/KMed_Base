...
if (!empty($reg)) {
  ...
  foreach ($reg as $entry) {
    ...
    $enqc=$GLOBALS['encounter'];

	$sqql="select pc_catid from form_encounter where encounter='".$enqc."'";
	$sqql2="select pid,status from t_form_admit where encounter='".$enqc."'";
	$rsqql2=sqlStatement($sqql2);
	$rowsql2=sqlFetchArray($rsqql2);

	$rsqql=sqlStatement($sqql);
	$rowsql=sqlFetchArray($rsqql);
	$catg=$rowsql['pc_catid'];
	$stat2=$rowsql2['status'];
    if ($nickname=='Admission')
    	{
    		if($catg==12 &&  $stat2!='discharge')
    			{
