...
jQuery.ajax({
type: 'POST',
url: "http://103.230.38.89/kavaii/interface/new/insertzip.php", //file which read zip code excel file
dataType: "json", //is used for return multiple values
data: { 's' : s },
success: function(data){
try {
var test=data.arealist;	
jQuery("#form_city").val(data.dist);//city-class of city text filed
jQuery("#form_locality").val(""); //has arealist
jQuery("#form_state").val(data.state);
jQuery("#form_country_code").val('India');

$("#form_locality").html("");
//$(“#ComboBox”).html(“”);
$.each(test, function(index, obj) {
               $('#form_locality').append( $('<option></option>').val(obj).html(obj));
           });
...
jQuery('#form_title').blur(function(){
	var title = jQuery(this).val();
	if(title=="Mr.")
	{
		jQuery("#form_sex").val('Male');
	}else if(title=="Mrs.")
	{
		jQuery("#form_sex").val('Female');
	}else if(title=="Ms.")
	{
		jQuery("#form_sex").val('Female');
...
<form autocomplete='off' action='new_comprehensive_save.php' name='demographics_form' method='post' onsubmit='return validate(this)'>
 ...
 <tr>
  <td valign='top' colspan='2'>
   ...
   <select name="i<?php echo $i?>provider">
    <option value="" selected><?php xl(' Not Applicable','e'); ?></option>
<?php
