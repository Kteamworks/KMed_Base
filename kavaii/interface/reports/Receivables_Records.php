...
<title><?php xl('Bank Voucher Receipts','e'); ?></title>
...
<span class='title'><?php xl('Report','e'); ?> - <?php xl('Receivables Voucher Receipts','e'); ?></span>

<div id="report_parameters_daterange">
<?php echo date("d F Y", strtotime($form_from_date)) ." &nbsp; to &nbsp; ". date("d F Y", strtotime($form_to_date)); ?>
</div>

<form name='theform' method='post' action='Withdrawal_Records.php' id='theform'>
  ...
  <td width='410px'>
	...
	<table class='text'>
		<tr>
			...
			$ures = sqlStatement("SELECT distinct auth_user FROM vouchers_receivables where auth_user is not null");
			...
			<td>
				...
				<a href='#' class='css_button' onclick='$("#form_refresh").attr("value","true"); $("#form_csvexport").attr("value",""); $("#theform").submit();'>
                  ...
                  <?php if ($_POST['form_refresh'] || $_POST['form_csvexport']) { ?>
					...
					<a href='#' class='css_button' onclick='$("#form_csvexport").attr("value",""); $("#form_csvexport").attr("value","true"); $("#theform").submit();'>
						<span>
							<?php xl('CSV Export','e'); ?>
						</span>
					</a>
...
if ($usr=='all')
{
  ...
  $query = "SELECT id, voucher_no,posted_date,amount,head,auth_user, description, pay_to FROM vouchers_receivables WHERE " .
