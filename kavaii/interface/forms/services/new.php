...
if ($_POST['bn_save'] || $_POST['bn_xmit']) {
  ...
  for ($i = 0; isset($_POST['form_proc_type'][$i]); ++$i) {
	...
	{
	$qgetfee1=sqlStatement("SELECT a.code as cd ,a.code_text as cd_tx,a.code_type as cd_ty,b.pr_price as fee from codes a,prices b  where a.id=b.pr_id and a.code_type=6 and  b.pr_level='HosInsurance' and a.code='".$proc_desc."'");
   ...
   //$fee=$row2['pr_price'];
	...
	{
	$qgetfee=sqlStatement("SELECT a.code as cd ,a.code_text as cd_tx,a.code_type as cd_ty,b.pr_price as fee from codes a,prices b  where a.id=b.pr_id and a.code_type=6 and b.pr_level='standard' and a.code='".$proc_desc."'");
...
<form method="post" action="<?php echo $rootdir ?>/forms/services/new.php?id=<?php echo $formid ?>"
 onsubmit="return validate(this)">

<p class='title' style='margin-top:8px;margin-bottom:8px;text-align:center'>
<?php
  echo xl('Services for') . ' ';
...
?>
 <tr>
  <td width='1%' valign='top'><b><?php echo xl('Services') . ' ' . ($i + 1); ?>:</b></td>
  <td valign='top'>
