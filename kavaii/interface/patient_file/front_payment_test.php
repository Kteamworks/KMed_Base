...
$patdata = sqlQuery("SELECT " .
  "p.phone_cell,p.date,p.city,p.state,p.street,p.title,p.age,p.age_days,p.age_months,p.rateplan,p.date,p.DOB,p.sex,p.genericname1,p.fname, p.mname, p.lname, p.pubpid,p.pid, i.copay " .
...
if ($_POST['form_save']) {
  ...
  $total_amt=$_POST['f1_total'];
  $e=sqlStatement("select * from billing_activity_final where billid='$billid'");
  $R=sqlFetchArray($e);
  if($R['flag']==1)
  {
  if($R['claim_amt']!=null)
  {
  $claim_amt=$R['claim_amt'];
  
  }
  if($R['approved_amt']!=0.00 )
  {
  $approved_amt=$R['approved_amt'];
 // $q2='';
  }
  ...
  sqlStatement("update billing_activity_final set ".
  "claim_amt=?".
 ", auth_no=?".
  ", approved_amt=?".
  "where billid=? ",
   array($claim_amt,$auth_no,$approved_amt,$billid));
  
   sqlStatement("update billing_main_copy set ".
  "claim_amt=?".
  ", approved_amt=?".
  ",approval_date=now()".
  ",claim_date=now()".
  ...
  "where encounter=? ",
   array($claim_amt,$approved_amt,$encounter));
  ...
  else
  {
	  ...
	  $qqq1='';
	  if ($approved_amt!=null)
	  {
		$qqq1=", approval_date=now()";	  
	  }
$RSN = sqlStatement("Select max(date) bd
from billing 
where encounter=?",array($encounter));  

$r2=sqlFetchArray($RSN);
$r32=$r2['bd'];

$RSN1 = sqlStatement("Select discharge_date from t_form_admit 
where encounter=?",array($encounter));  
$pp=sqlStatement("select amount1+amount2 amt from payments where encounter=? group by encounter",array($encounter));
$pp=sqlFetchArray($pp);
$patpay=$pp['amt'];
$r22=sqlFetchArray($RSN1);
$r321=$r22['discharge_date'];

  $bil_act=idSqlStatement("INSERT into billing_activity_final set ".
  "pid=?".
  ", date=now()".
  ",encounter=?".
  ",by_patient_amt=?".
  ...
  $qqq1.
  ", approved_amt=?",
  array($pid,$encounter,$patpay,$billid,$total_amt,$claim_amt,$auth_no,$r32,$r321,$approved_amt));
  ...
  $patdata = getPatientData($form_pid, 'city,state,street,title,age,age_days,age_months,rateplan,date,sex,DOB,genericname1,fname,mname,lname,pubpid');
...
if ($_POST['form_save'] || $_REQUEST['receipt']) {
  ...
  $patdata = getPatientData($form_pid, 'phone_cell,date,city,state,street,title,age,age_days,age_months,rateplan,date,sex,DOB,genericname1,fname,mname,lname,pubpid');
...
$rateplan=$patdata['rateplan'];
	...
	echo "<tr><td  style='padding-right: 100px;' >" . xlt('Phone Number:') . " " .text($patdata['phone_cell'])."</td>";
    ...
    echo "<td  style='padding-right: 10px;' >" . xlt('Rate Plan') . ": &nbsp&nbsp" . text($patdata['rateplan']) . "</td></tr>";
	}
	if($rateplan=="TPAInsurance")
	{
	echo "<tr><td  style='padding-right: 100px;' >" . xlt('TPA Insurance') . ": ".$insurance1['name'] ."</td>";
	echo "<td  style='padding-right: 10px;' >" . xlt('Pre-Auth No.') . ": " . text($auth_no) . "</td></tr>";
	}
            ...
            //foreach ($patient as $be) {
				...
				foreach ($billing as $b)				
				{
				 ...
				 $bdate = strtotime($b['date']);
					...
					$counta=sqlStatement("SELECT sum(fee) as st ,code_type, count(code_type) as c from billing where encounter='".$encounter."' and activity='1' and code_type='".$ct."'");
...
//		   echo "<tr style='border-top: 1px solid #000;'><td colspan=6>&nbsp; </td></tr>";
	...
	echo "<tr style='border-top: 1px solid #000;' ><td></td><td></td><td></td><td></td><td class='bold' style='border-bottom: 1px solid #000;' nowrap style='text-align:right'>".xlt('Net Payments ')."</td><td class='text' style='border-bottom: 1px solid #000;'align='right' >" ."". oeFormatMoney($nettotal) . "</td></tr>";
   echo "<tr><td></td><td></td><td></td><td></td><td class='bold' style='border-bottom: 0px solid #000;'  style='align:right' nowrap>".xlt('Total Bill Amount')."</td><td class='text' align='right'>" . oeFormatMoney($total) . "</td></tr>";
