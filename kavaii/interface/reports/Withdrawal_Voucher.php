...
<title><?php echo xlt('Withdrawal Voucher') ?></title>
</head>
<body leftmargin='0' topmargin='0' marginwidth='0' marginheight='0' class="body_top">


<form method='post' action='Withdrawal_Voucher.php' id='theform'>
	...
	<table class='text'>
		<tr>
			...
			<?php dropdown_facility($form_facility, 'form_facility', true); ?>
			                        ...
			                        <td><?php echo xlt('Received From'); ?>:</td>
                <td><?php
				...
				<td><input type='text' name='amt' size=8></input>  </td>       
						...
						<td><?php echo xlt('Payment Mode ');?><select name="form_paymode"id="form_paymode">
<option value="Cash">Cash</option>
<option value="Check">Cheque</option>
</select>
<td> Cheque Number:
						<input type="text" name="check_no" size=10 id="check_no"></input></td>
</td>
<td><?php echo xlt('Bank Name'); ?>:</td>
                <td><?php
                        // Build a drop-down list of providers.
                                //
                                $query = "SELECT title FROM list_options  WHERE ".
...
<td><a href='add_new_bank.php'>Add new Bank</a></td>
</tr>
...
if($_POST['form_save'])
{
 $sqlBindArray=array();
	...
	$form_paymode=$_POST['form_paymode'];
...
$rse=$rs['username'];
	...
	$payout=$_POST['amt'];
	 ...
	 $r2=sqlInsert("INSERT into vouchers_withdrawal SET ".
	  "posted_date= NOW(), " .
	  "pay_to = '" . add_escape_custom($pr) . "', " .
	  "description = '" . add_escape_custom($_POST['form_des']) . "', " .
	  "amount= '" . add_escape_custom($Totalamt) . "', " .
	  "bank= '" . add_escape_custom($_POST['Banks']) . "', " .
	 "payment_mode = '" . add_escape_custom($form_paymode) . "', " .
	"reference_no = '" . add_escape_custom($check_no) . "', " .
	  ...
	  $voucher_no="SELECT voucher_no from vouchers_withdrawal order by voucher_no desc limit 1";
	  $voucher=sqlStatement($voucher_no);
	  $voch=sqlFetchArray($voucher);
	  $voc=$voch['voucher_no'];
	echo "<center><h4>".xlt("Withdrawal Voucher")."</h4></center>";
	?>  
<div id="report">
...
<tr> </tr>
<tr height="30">
<td>Payment Mode:<?php echo text($form_paymode)?></td>
<td align="right">Reference No:<?php echo text($check_no)?></td>
</tr>
<tr> </tr>
<tr> </tr>
<tr> </tr>
...
<input type='hidden' name='Banks' value="<?php echo "".text($Banks).""; ?>"> </input>
<td></td>
<td></td>
...
<input type='hidden' name='check_no' id='check_no' value='<?php echo $_POST['check_no']?>'></input>
...
$bgcolor = ((++$orow & 1) ? "#ffdddd" : "#ddddff");
  ...
  $points = ($point) ?
    "." . $words[$point / 10] . " " . 
		...
		echo "<td>".text("Bank Name ").text($_POST['Banks'])."</td>";
		echo "<td>".text("Reference No:  ").text($_POST['check_no'])."</td></tr>";
