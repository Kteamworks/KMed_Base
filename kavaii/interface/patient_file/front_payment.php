	...
	if($_REQUEST['radio_type_of_payment']=='pre_payment')
	 {
		 ...
		 frontPayment($form_pid, 0, $form_method, $form_source, $_REQUEST['form_prepayment'], 0, $timestamp,$form_towards);//insertion to 'payments' table.
...
//----------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------------------------------------------------------------------------
					...
					sqlQuery("Update billing_main_copy set patient_paid=patient_paid + ?  where encounter=?",array($amount,$enc));   					
					 if($form_towards== 2)
					{
	  
					
					sqlQuery("Update billing_main_copy set bill_id=concat('BL','',encounter), bill_date = now(),PFYN_Flag=1 where encounter=?",array($enc));   
