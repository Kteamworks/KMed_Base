...
<?php
  ...
  if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
    ...
    if ($INTEGRATED_AR) {
		...
		$query="SELECT ".
                    ...
                    " (select sum(amount) from vouchers  ".
					 ...
					 " (select sum(amount) from vouchers_bank  ".
                     " where posted_date >= '$from_date 00:00:00' and  posted_date<='$to_date 23:59:59') AS bank_expenses";
    ...
    echo '"' . xl('Expenses:'). '",';
	echo '"' .oeFormatMoney($row['expenses']+$row['bank_expenses']) . '"'."\n";
	echo '"' . xl('Margin:'). '",';
	echo '"' .oeFormatMoney($row['REV']-$row['expenses']-$row['discounts']-$row['bank_expenses']) . '"'."\n";
   } else{?>
...
<td class="detail"><b><?php xl('Total Revenue','e'); ?></b></td>
  ...
  <td align="right">
   <?php bucks($row['expenses']+$row['bank_expenses']); ?>
  </td>
 </tr>
  <tr bgcolor="#ddddff">
  <td class="detail"><b><?php xl('Margin','e'); ?></b></td>
  <td align="right">
   <?php bucks($row['REV']-$row['expenses']-$row['discounts']-$row['bank_expenses']); ?>
  </td>
 </tr>
